...
class BroadcastGlobalVariablesCallbackImpl(object):
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        bcast_op = keras_broadcast_global_variables(self.root_rank)
        if size() > 1:
...
class MetricAverageCallbackImpl(object):
    ...
    #        allreduce_op = keras_allreduce(value, name=name)
    ...
    def _average_metrics_in_place(self, logs):
        ...
        _allreduce_fn = keras_allreduce_fn(name="Metric")
        for metric, value in sorted(logs.items()):

            if metric not in self.variables:
                self.variables[metric] = self.backend.variable(value, name=metric)
                self.backend.get_session().run(self.variables[metric].initializer)
            else:
                self.backend.set_value(self.variables[metric], value)
        
        #print(logs)
        log_metrics = []
        for metric, _ in sorted(logs.items()):
            log_metrics.append(self.variables[metric])
        allreduce_metrics = _allreduce_fn(log_metrics)
        allreduce_metrics_value = self.backend.get_session().run(allreduce_metrics)

        counter = 0
        for metric, _ in sorted(logs.items()):
            reduced_logs[metric] = allreduce_metrics_value[counter]
            counter = counter + 1
        #print(reduced_logs)
...
class LearningRateWarmupCallbackImpl(LearningRateScheduleCallbackImpl):
    def __init__(self, backend, warmup_epochs=5, momentum_correction=True, steps_per_epoch=None,
        ...
        def multiplier(epoch):
            ...
            return 1. / size() * (epoch * (size() - 1) / warmup_epochs + 1)
